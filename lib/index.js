	...
	self._getInfo = function(opt) {
		...
		try {
			if(opt.artist == undefined || opt.artist == '' && typeof opt.callback == 'function') {
				...
				var options = {
					url: 'http://ws.audioscrobbler.com/2.0/?method=' + (opt.track != undefined && opt.track != '' ? 'track' : 'artist') + '.getinfo&api_key=' + this.api_key + '&autocorrect=1&username=' + this.username + '&artist=' + encodeURIComponent(opt.artist) + (opt.track == undefined || opt.track == '' ? '' : '&track=' + encodeURIComponent(opt.track))
				};
...
//			console.log("result: ", result);
			if(typeof the_callback == 'function') {
				if(result['@'].status == 'ok') {
					the_callback({
						success: true,
						tags: opt.track != undefined && opt.track != '' ? result.track.toptags.tag : result.artist.tags.tag
					});
				} else {
					the_callback({
						success: false,
