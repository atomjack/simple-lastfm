...
Lastfm.prototype.getSessionKey = function(callback) {
	...
	http.get({
		host: 'ws.audioscrobbler.com',
		port: 80,
		path: '/2.0/?method=auth.getMobileSession&' + 'username=' + this.username + '&' + 'authToken=' + authToken + '&' + 'api_key=' + this.api_key + '&' + 'api_sig=' + api_sig
	}, function(res) {
		var body = '';
		res.on('data', function(chunk) {
			body += chunk;
...
Lastfm.prototype.getTopTracks = function(opt) {
	var lastfm = this;
	var the_callback = opt.callback;
	delete opt.callback;
	lastfm.doGet({
		...
		callback: function(result) {
			if (typeof the_callback === 'function') {
				if (result['@'].status == 'ok') {
					the_callback({
						success: true,
						topTracks: result.toptracks.track
					});
				} else {
					the_callback({
						success: false,
...
Lastfm.prototype.getRecentTracks = function(opt) {
	var lastfm = this;
	var the_callback = opt.callback;
	delete opt.callback;
	lastfm.doGet({
		...
		callback: function(result) {
			if (typeof the_callback === 'function') {
				if (result['@'].status == 'ok') {
					the_callback({
						success: true,
						recentTracks: result.recenttracks.track
					});
				} else {
					the_callback({
						success: false,
